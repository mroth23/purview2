...
shared_ptr<vector<vector<vector<float>>>> convolve(vector<const float[][]> Kernels, vector<vector<float>> Matrix, unsigned long Width, unsigned long Height) {
    ...
    for (unsigned X = 0; X < Width; ++X) {
        for (unsigned Y = 0; Y < Height; ++Y) {
            ...
            vector<float> Sum(Kernels.size(), 0f);
